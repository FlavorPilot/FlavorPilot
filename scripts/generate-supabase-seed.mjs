...
for(const p of preparationMethods)sql+=`insert into public.preparation_methods(id,name_en,name_uk,profile_multiplier,intensity_multiplier,add_aromas,add_textures,source,review_status,model_version) values(${[q(p.id),q(p.name.en),q(p.name.uk),json(p.profileMultiplier),p.intensityMultiplier,arr(p.addAromas??[]),arr(p.addTextures??[]),q(note),q('unreviewed'),q('0.3.0-hypothesis')].join(',')}) on conflict (id) do nothing;\n`;
for(const i of ingredients)sql+=`insert into public.ingredients(id,name_en,name_uk,category_en,category_uk,sensory_profile,intensity,texture_intensity,aromas,textures,roles,min_share,ideal_share,max_share,preparation_ids,source_note,source,review_status,model_version,confidence) values(${[q(i.id),q(i.name.en),q(i.name.uk),q(i.category.en),q(i.category.uk),json(i.profile),i.intensity,i.textureIntensity,arr(i.aromas),arr(i.textures),arr(i.roles),i.share.min,i.share.ideal,i.share.max,arr(i.preparations),q(note),q(note),q('unreviewed'),q('0.3.0-hypothesis'),0].join(',')}) on conflict (id) do nothing;\n`;
for(const [key,value] of explicitPairAdjustments){const [a,b]=key.split('::');sql+=`insert into public.ingredient_pairings(ingredient_a_id,ingredient_b_id,explicit_adjustment,source_note,source,review_status,model_version,confidence) values(${q(a)},${q(b)},${value},${q(note)},${q(note)},${q('unreviewed')},${q('0.3.0-hypothesis')},0) on conflict (ingredient_a_id,ingredient_b_id) do nothing;\n`;}
