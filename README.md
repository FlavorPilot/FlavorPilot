...
В репозитории есть проверенный `package-lock.json`. Устанавливайте зависимости так:

```bash
npm ci --include=optional
```

Целевая среда CI — Node.js 22. Локальная проверка 23 сентября 2026 выполнена на Node.js 24.21.0
и npm 11.19.0: `npm ci` завершился, но npm предупредил о несовпадении `engines`.
Не применяйте `--force`, `--legacy-peer-deps` и случайные SWC bindings ради обхода ошибок.

Изменения вносятся через pull request в `main`. Прямой push в `main` не используется:
зелёные `npm run validate` и `npm run build` на PR — условие принятия изменения.
...
`npm run validate` и `npm run build` прошли 23 сентября 2026 на Node.js 24.21.0 / npm 11.19.0.
Подробности и границы проверки — в [validation-report.json](validation-report.json).
Эти команды не заменяют тесты настоящей БД, браузерные E2E и проверку деплоя.
