...
create table public.ingredients (
 id text primary key,name_en text not null,name_uk text not null,category_en text not null,category_uk text not null,
...
create table public.dishes (
 ...
 name text not null check (char_length(name) between 1 and 160),goal text not null default 'balanced' check (goal in ('balanced','fresh','rich','spicy','sweetSour','smoky','umami','light','creamy','crunchy')),
 visibility public.dish_visibility not null default 'private',share_token uuid not null default gen_random_uuid(),parent_dish_id uuid references public.dishes(id) on delete set null,
 description text check (description is null or char_length(description)<=4000),image_url text check (image_url is null or char_length(image_url)<=2048),published_at timestamptz,created_at timestamptz not null default now(),updated_at timestamptz not null default now()
);
create table public.dish_items (
 id uuid primary key default gen_random_uuid(),dish_id uuid not null references public.dishes(id) on delete cascade,ingredient_id text not null references public.ingredients(id),
 grams numeric(10,3) not null check (grams>0 and grams<=5000),preparation_id text not null references public.preparation_methods(id),position smallint not null default 0 check (position>=0),note text check (note is null or char_length(note)<=2000),created_at timestamptz not null default now()
);
create table public.dish_versions (
 id uuid primary key default gen_random_uuid(),dish_id uuid not null references public.dishes(id) on delete cascade,version_number integer not null check (version_number>0),snapshot jsonb not null,created_by uuid not null references public.profiles(id),created_at timestamptz not null default now(),unique(dish_id,version_number)
);
create table public.favorites (
 user_id uuid not null references public.profiles(id) on delete cascade,dish_id uuid not null references public.dishes(id) on delete cascade,created_at timestamptz not null default now(),primary key(user_id,dish_id)
);
create table public.subscriptions (
 user_id uuid primary key references public.profiles(id) on delete cascade,tier public.subscription_tier not null default 'free',provider_customer_id text,provider_subscription_id text,
 status text not null default 'inactive' check (status in ('inactive','trialing','active','past_due','canceled')),current_period_end timestamptz,updated_at timestamptz not null default now()
);
...
create unique index dish_items_unique_ingredient_idx on public.dish_items(dish_id,ingredient_id,preparation_id);
...
create or replace function public.validate_dish_item_catalog() returns trigger language plpgsql security definer set search_path=public as $$
begin
 if not exists(select 1 from public.ingredients i where i.id=new.ingredient_id and new.preparation_id=any(i.preparation_ids)) then raise exception 'UNSUPPORTED_DISH_ITEM' using errcode='P0001';end if;
 if tg_op='INSERT' then
  perform 1 from public.dishes d where d.id=new.dish_id for update;
  if (select count(*) from public.dish_items di where di.dish_id=new.dish_id)>=24 then raise exception 'DISH_ITEM_LIMIT_REACHED' using errcode='P0001';end if;
...
create trigger dish_items_validate_catalog before insert or update of dish_id,ingredient_id,preparation_id on public.dish_items for each row execute function public.validate_dish_item_catalog();

create or replace function public.normalize_dish_publication() returns trigger language plpgsql as $$
begin
 if new.visibility='public' then
  if tg_op='INSERT' then new.published_at=now();
  elsif old.visibility is distinct from 'public' then new.published_at=now();
  else new.published_at=old.published_at;end if;
 else new.published_at=null;end if;
 return new;
end;$$;
create trigger dishes_normalize_publication before insert or update of visibility,published_at on public.dishes for each row execute function public.normalize_dish_publication();

create or replace function public.validate_dish_parent() returns trigger language plpgsql security definer set search_path=public as $$
begin
 if new.parent_dish_id is null then return new;end if;
 if new.parent_dish_id=new.id then raise exception 'INVALID_PARENT_DISH' using errcode='P0001';end if;
 if not exists(select 1 from public.dishes parent where parent.id=new.parent_dish_id and (parent.visibility='public' or parent.owner_id=new.owner_id)) then raise exception 'INVALID_PARENT_DISH' using errcode='P0001';end if;
 return new;
end;$$;
create trigger dishes_validate_parent before insert on public.dishes for each row execute function public.validate_dish_parent();
create or replace function public.preserve_dish_parent_lineage() returns trigger language plpgsql security definer set search_path=public as $$
begin
 if old.parent_dish_id is not distinct from new.parent_dish_id then return new;end if;
 if new.parent_dish_id is null and old.parent_dish_id is not null and not exists(select 1 from public.dishes parent where parent.id=old.parent_dish_id) then return new;end if;
 raise exception 'IMMUTABLE_PARENT_DISH' using errcode='P0001';
end;$$;
create trigger dishes_preserve_parent_lineage before update of parent_dish_id on public.dishes for each row execute function public.preserve_dish_parent_lineage();
...
declare requested_username text;base_username text;
begin
 requested_username=lower(regexp_replace(coalesce(new.raw_user_meta_data->>'username',''),'[^a-z0-9_]+','','g'));
 if char_length(requested_username)<3 then requested_username='chef_'||substring(new.id::text from 1 for 8);end if;
 base_username=left(requested_username,23);
 if exists(select 1 from public.profiles p where p.username=base_username) then base_username=base_username||'_'||substring(replace(new.id::text,'-','') from 1 for 8);end if;
 insert into public.profiles(id,username,display_name) values(new.id,base_username,coalesce(nullif(new.raw_user_meta_data->>'display_name',''),base_username));
...
create trigger dishes_enforce_private_limit before insert or update of visibility,owner_id on public.dishes for each row execute function public.enforce_private_dish_limit();
...
create policy "Ingredient knowledge is public" on public.ingredients for select using(true);
create policy "Preparation knowledge is public" on public.preparation_methods for select using(true);
create policy "Pairing knowledge is public" on public.ingredient_pairings for select using(true);
create policy "Public dishes or owner can read" on public.dishes for select using(visibility='public' or owner_id=auth.uid());
create policy "Owners create dishes" on public.dishes for insert with check(owner_id=auth.uid());
create policy "Owners update dishes" on public.dishes for update using(owner_id=auth.uid()) with check(owner_id=auth.uid());
create policy "Owners delete dishes" on public.dishes for delete using(owner_id=auth.uid());
create policy "Readable dish items" on public.dish_items for select using(exists(select 1 from public.dishes d where d.id=dish_items.dish_id and (d.visibility='public' or d.owner_id=auth.uid())));
create policy "Owners create dish items" on public.dish_items for insert with check(exists(select 1 from public.dishes d where d.id=dish_items.dish_id and d.owner_id=auth.uid()));
create policy "Owners update dish items" on public.dish_items for update using(exists(select 1 from public.dishes d where d.id=dish_items.dish_id and d.owner_id=auth.uid())) with check(exists(select 1 from public.dishes d where d.id=dish_items.dish_id and d.owner_id=auth.uid()));
create policy "Owners delete dish items" on public.dish_items for delete using(exists(select 1 from public.dishes d where d.id=dish_items.dish_id and d.owner_id=auth.uid()));
create policy "Owners read versions" on public.dish_versions for select using(exists(select 1 from public.dishes d where d.id=dish_versions.dish_id and d.owner_id=auth.uid()));
create policy "Owners create versions" on public.dish_versions for insert with check(created_by=auth.uid() and exists(select 1 from public.dishes d where d.id=dish_versions.dish_id and d.owner_id=auth.uid()));
create policy "Users read own favorites" on public.favorites for select using(user_id=auth.uid());
create policy "Users create own favorites" on public.favorites for insert with check(user_id=auth.uid());
...
create table public.ingredient_nutrient_hypotheses (
 ...
 protein_g numeric,protein_nutrient_id integer,carbohydrate_g numeric,carbohydrate_nutrient_id integer,energy_kcal numeric,energy_nutrient_id integer,
 saltiness numeric,fat_score numeric,sweetness numeric,moisture numeric,
 reviewer text,review_status text not null default 'unreviewed' check (review_status in ('unreviewed','in_review','reviewed','rejected')),
 confidence numeric(4,3) not null default 0 check (confidence between 0 and 1),source_license text not null,last_reviewed_at timestamptz,
 ...
 constraint hypotheses_composition_amounts_nonnegative check ((protein_g is null or protein_g >= 0) and (carbohydrate_g is null or carbohydrate_g >= 0) and (energy_kcal is null or energy_kcal >= 0)),
 constraint hypotheses_protein_id_follows check ((protein_nutrient_id is null) = (protein_g is null) and (protein_nutrient_id is null or protein_nutrient_id = 1003)),
 constraint hypotheses_carbohydrate_id_follows check ((carbohydrate_nutrient_id is null) = (carbohydrate_g is null) and (carbohydrate_nutrient_id is null or carbohydrate_nutrient_id in (1005, 1050))),
 constraint hypotheses_energy_id_follows check ((energy_nutrient_id is null) = (energy_kcal is null) and (energy_nutrient_id is null or energy_nutrient_id in (1008, 2047, 2048)))
);
