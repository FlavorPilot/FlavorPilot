...
export function PlanComparison({ locale }: {
    ...
    return <div className="plans-grid"><section className="plan-card"><Badge tone="green">{t.available}</Badge><h2>{t.free}</h2><p>{t.freeDesc}</p><strong className="plan-price">€0</strong><ul>{[t.freeFeature1, t.freeFeature2, t.freeFeature3, t.freeFeature4].map(feature => <li key={feature}><Icon name="check" size={18}/>{feature}</li>)}</ul><LinkButton href={`/${locale}/builder`} variant="primary">{t.openStudio}<Icon name="arrow" size={16}/></LinkButton></section>
